...
class CustomerSiteService
{
    public function validateCustomerSite(array $data, bool $isUpdate = false): array
    {
        $rules = [
            ...
            'site_address' => ($isUpdate ? 'sometimes|' : 'required|') . 'string|max:255',
            'site_name' => ($isUpdate ? 'sometimes|' : 'required|') . 'string|max:255',
            'phone_number' => ($isUpdate ? 'sometimes|' : 'required|') . 'string|max:20',
            'email' => ($isUpdate ? 'sometimes|' : 'required|') . 'email|max:255',
            'site_contact_person_name' => ($isUpdate ? 'sometimes|' : 'required|') . 'string|max:255',
            'site_contact_person_email' => ($isUpdate ? 'sometimes|' : 'required|') . 'email|max:255',
            'site_contact_person_phone_number' => ($isUpdate ? 'sometimes|' : 'required|') . 'string|max:20',
            'site_contact_person_signature' => 'nullable|string',
            'site_existing_status' => ($isUpdate ? 'sometimes|' : 'required|') . 'boolean',
